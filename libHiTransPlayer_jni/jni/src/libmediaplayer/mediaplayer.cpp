...
status_t MediaPlayer::prepare()
{
...
#ifdef HI_MEDIA_CODEC_DECODING
    if (apiVer < 21)
    {
        MMLOGE(TAG, "API version not supported \n");
        goto ProtoStop;
    }
#else
    if (apiVer < 14)
    ...
    else
    {
        ...
        {
            mMediaHandlr = new HiMediaCodecHandlr(mjsurface, mjenv, mAPIVersion, mClient, mPCMWriter);
        }
...
status_t MediaPlayer::setDataSource(const char* url)
{
    ...
    else if (strcasestr(url, "udp://"))
    {
        mClient = new HiUdpClient(url);

        if (mSaveVideoFlag)
        { (static_cast<HiUdpClient*>(mClient))->setVideoSaveFlag(mSaveVideoFlag); }
    }
    else
    {
    ...
    if (mListener)
    { mClient->setListener(this); }  //该监听函数在基类里调用
    else
    {
...
void* MediaPlayer::StatusLooper(const void* args)
{
    ...
    {
        LowWaterLevel = 10 * 1024;
        HighWaterLevel = 512 * 1024;
    }
...
{
    /*ע��surface ��Native Window, ������windowʵ��*/
...
status_t MediaPlayer::reset()
{
    ...
    MMLOGI(TAG, "Lorin add ----------mclient  disconnect  finish-------------\n");
    if (mMediaHandlr)
    {
     ...
     MMLOGI(TAG, "Lorin add ----------mMediaHandlr  stop close delete  finish-------------\n");
    if (mClient)
    {
        if (mCacheSource)
        {
        ...
        delete mClient;
        mClient = NULL;
         MMLOGI(TAG, "Lorin add ----------mClient   delete  finish-------------\n");
...
void MediaPlayer::setRecordFlag(int flag)
{
    ...
    if (mClient && mClient->getProtoType() == PROTO_TYPE_UDP_CLIENT)
    { static_cast<HiUdpClient*>(mClient)->setRecordFlag(flag); }
}
...
{
    int iRet = -1;
    unsigned int uBufSize = (unsigned int)dataSize;
    ...
    else if(mClient && mClient->getProtoType() == PROTO_TYPE_UDP_CLIENT)
    {
        iRet = static_cast<HiUdpClient*>(mClient)->getRecordVideo(ptr, uBufSize, pts, ptype);
        if (iRet != 0)
        {
            return iRet;
        }
...
status_t MediaPlayer::getRecordAudio(void* ptr, int& dataSize, int64_t& pts, int& ptype)
{
    ...
    {
        iRet = static_cast<HiUdpClient*>(mClient)->getRecordAudio(ptr, uBufSize, pts, ptype);
        if (iRet != 0)
        {
            return iRet;
        }
